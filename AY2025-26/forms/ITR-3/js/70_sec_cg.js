...
function scripBlock(rows,suf){
  ...
  /* 3b ADD (AY2025-26): Schedule112A adds a per-row ShareTransferredOnOrBefore flag
     (BE = transferred before 23-07-2024 @ 10% indexed, AE = on/after @ 12.5%) and the
     top-level Balance112A AE/BE split with its total. This golden's 112A scrips are
     transferred post-23-07-2024, so the balance falls entirely in AE. */
  if(suf==="112A") dtls.forEach((d,i)=>{d.ShareTransferredOnOrBefore=(good[i].after23==="BF")?"BE":"AE";});
  ...
  if(suf==="112A"){const beBal=good.reduce((a,r,i)=>a+((r.after23==="BF")?N(sg(dtls[i].Balance)):0),0);
    b.Balance112ABE=n0(beBal); b.Balance112AAE=sg(N(t.bal)-beBal); b.TotalBalance112A=sg(t.bal);}
  return b;
}
...
function expCg(j){
  ...
  /* 3b ADD (AY2025-26): the 23-July-2024 transfer split — AE = on/after 23-07-2024
     (12.5%), BE = before (10% with indexation). This golden's 112A gain is post-
     transition, so AE carries it and BE is nil. */
  LT.SaleOfEquityShareUs112A={BalanceCG:sg(B.b4.a),DeductionUs54F:n0((C.b4||{}).s54F),CapgainonAssets:sg(B.b4.gain),
    CapgainonAssetsTransferAE:sg(B.b4.gain),CapgainonAssetsTransferBE:0,
    BalanceCGTransferAE:sg(B.b4.a),BalanceCGTransferBE:0,
    DeductionUs54FAE:n0((C.b4||{}).s54F),DeductionUs54FBE:0};
  ...
  LT.PassThrIncNatureLTCGUs112A=n0((B.b11||{}).gain112a);   /* 3b ADD (AY2025-26): pass-through 112A LTCG (nil here) */
  ...
  /* 3b ADD (AY2025-26): NRI 48-proviso STT-paid AE/BE 23-07-2024 transfer split (nil here). */
  dfl(ST,"NRITransacSec48Dtl",{NRItaxSTTPaid:0,NRItaxSTTNotPaid:0,NRItaxSTTPaidTransferAE:0,NRItaxSTTPaidTransferBE:0});
  ...
  /* 3b ADD (AY2025-26): NRI 112A / foreign-asset AE/BE 23-07-2024 transfer split (nil here). */
  dfl(LT,"NRISaleOfEquityShareUs112A",{BalanceCG:0,DeductionUs54F:0,CapgainonAssets:0,
    CapgainonAssetsTransferAE:0,CapgainonAssetsTransferBE:0,BalanceCGTransferAE:0,BalanceCGTransferBE:0,
    DeductionUs54FAE:0,DeductionUs54FBE:0});
  dfl(LT,"NRISaleofForeignAsset",{SaleonSpecAsset:0,DednSpecAssetus115:0,BalonSpeciAsset:0,
    SaleonSpecAssetTransferAE:0,SaleonSpecAssetTransferBE:0,DednSpecAssetus115AE:0,DednSpecAssetus115BE:0});
  ...
  /* ---- Part E ---- */
  /* 3b ADD (AY2025-26): the 23-July-2024 rate transition adds the STCG-15% and
     LTCG-10%/20% (pre-transition) rate buckets and their set-off columns. The
     engine carries no pre-transition figures for this golden, so they emit as 0. */
  const SL=[["st15","InStcg15Per","StclSetoff15Per"],["st20","InStcg20Per","StclSetoff20Per"],["st30","InStcg30Per","StclSetoff30Per"],
    ["stApp","InStcgAppRate","StclSetoffAppRate"],["stDTAA","InStcgDTAARate","StclSetoffDTAARate"],
    ["lt10","InLtcg10Per","LtclSetOff10Per"],["lt125","InLtcg12_5Per","LtclSetOff12_5Per"],["lt20","InLtcg20Per","LtclSetOff20Per"],["ltDTAA","InLtcgDTAARate","LtclSetOffDTAARate"]];
  ...
  /* 3b ADD (AY2025-26): Part F gains the ShortTermUnder15Per and LongTermUnder10Per/20Per
     transition rate buckets. */
  const FN={st15:"ShortTermUnder15Per",st20:"ShortTermUnder20Per",st30:"ShortTermUnder30Per",stApp:"ShortTermUnderAppRate",
    stDTAA:"ShortTermUnderDTAARate",lt10:"LongTermUnder10Per",lt125:"LongTermUnder12_5Per",lt20:"LongTermUnder20Per",ltDTAA:"LongTermUnderDTAARate"};
