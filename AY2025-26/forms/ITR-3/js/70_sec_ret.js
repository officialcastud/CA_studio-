...
if(S.fs.duedate===undefined)S.fs.duedate=YC.dueDefault;    /* FilingStatus.ItrFilingDueDate (year overlay) */
...
const DUE_DATES=YC.dueDates;   /* year overlay 05_year_config — [2025-07-31, 2025-10-31, 2025-11-30] */
const YN=[["Y","Yes"],["N","No"]];
...
function secRet(){
  ...
  if(S.fs.optout==="Yes"){
    if(S.fs.incBP==="Y"){
      ...
      h+=row("Date of filing of Form 10-IEA for AY "+YC.ay,dte("fs.f10ieaDate"),{req:1,ind:1,ref:"O77"});
    ...
    if(S.fs.f10ieaEarlier==="Y"){
      ...
      h+=row("Assessment Year of that Form 10-IEA",sel("fs.f10ieaEarlierAY",[["2024-25","2024-25"]]),{ind:1,ref:"O65"});
    }
  }
  h+=regimeTable();
...
function chkRet(){
  ...
  if(S.fs.optout==="Yes"&&S.fs.incBP==="Y"){
    ...
    if(!D(S.fs.f10ieaDate)) out.push({lvl:"err",t:"Form 10-IEA date required",m:"Furnish the date of filing of Form 10-IEA for AY "+YC.ay+".",sec:"ret"});
