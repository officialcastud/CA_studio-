...
function secWho(){
  ...
  h+=row(ind?"Date of birth (DD/MM/YYYY)":"Date of formation (DD/MM/YYYY)",dte("pi.dob"),
    {req:1,ref:"W9 · DOB",hint:"on or before 31 March "+YC.fyEndYear});
...
function expWho(j){
  ...
  /* 3a REMOVE (AY2025-26): PersonalInfo.SecondaryAdd and the whole AlternateAddress
     block were dropped from the AY2025-26 ITR-3 schema — no longer emitted. */
  ...
  /* 3d CHANGE (AY2025-26): the AY2026-27 regime leaves (IncFrmBusOrProf,
     OptOldRegimeCurrAY, F10IEA*CurrAYOldTax) are replaced by the AY2025-26
     115BAC(6) opt-out machinery: OptOutNewTaxRegime_Method (BY10IEA when a
     business filer opts out via Form 10-IEA, else OPTINRETURN) plus the current-
     AY Form10IEADate/Form10IEAAckNo and the earlier-AY (2024-25) 10-IEA block. */
  put(j,"PartA_GEN1.FilingStatus.OptOutNewTaxRegime_Method",
      (fs.optout==="Yes"&&fs.incBP==="Y")?"BY10IEA":"OPTINRETURN");
  if(fs.optout==="Yes"&&fs.incBP==="Y"){
    put(j,"PartA_GEN1.FilingStatus.Form10IEADate",ISO(fs.f10ieaDate));
    if(fs.f10ieaAck) put(j,"PartA_GEN1.FilingStatus.Form10IEAAckNo",R(fs.f10ieaAck));
  }
  if(fs.f10ieaEarlier){
    put(j,"PartA_GEN1.FilingStatus.OptOutNewTaxRegime_Form10IEA_AY24_25",sv(fs.f10ieaEarlier));
    if(fs.f10ieaEarlierAck) put(j,"PartA_GEN1.FilingStatus.Form10IEAAckNo_AY24_25",R(fs.f10ieaEarlierAck));
    if(fs.f10ieaEarlierDate) put(j,"PartA_GEN1.FilingStatus.Form10IEADate_AY24_25",ISO(fs.f10ieaEarlierDate));
  }
  ...
  put(j,"PartA_GEN1.FilingStatus.ItrFilingDueDate",sv(fs.duedate||YC.dueDefault));
  if(fs.lei){
...
function impWho(I3){
  ...
  /* 3a REMOVE (AY2025-26): SecondaryAdd / AlternateAddress dropped — nothing to import. */
  ...
  /* 3d CHANGE (AY2025-26): regime read from OptOutNewTaxRegime_Method (present ⇒ old regime);
     current-AY 10-IEA from Form10IEADate/Form10IEAAckNo, earlier-AY from the _AY24_25 block. */
  if(FS.OptOutNewTaxRegime_Method!=null){S.fs.optout=(FS.OptOutNewTaxRegime_Method==="BY10IEA")?"Yes":"No";read.push("regime");}
  if(FS.Form10IEADate)S.fs.f10ieaDate=dmy(FS.Form10IEADate);
  if(FS.Form10IEAAckNo!=null)S.fs.f10ieaAck=String(FS.Form10IEAAckNo);
  if(FS.OptOutNewTaxRegime_Form10IEA_AY24_25!=null)S.fs.f10ieaEarlier=FS.OptOutNewTaxRegime_Form10IEA_AY24_25;
  if(FS.Form10IEAAckNo_AY24_25!=null)S.fs.f10ieaEarlierAck=String(FS.Form10IEAAckNo_AY24_25);
  if(FS.Form10IEADate_AY24_25)S.fs.f10ieaEarlierDate=dmy(FS.Form10IEADate_AY24_25);
...
function chkWho(){
  ...
  /* Date of birth / formation — mandatory, on or before 31 March of the P.Y. end (year overlay) */
  const dob=D(P.dob);
  if(!dob) out.push({lvl:"err",t:(ind?"Date of birth required":"Date of formation required"),m:"Enter the date in DD/MM/YYYY.",sec:"who"});
  else if(dob>YREND) out.push({lvl:"err",t:(ind?"Date of birth":"Date of formation"),m:"The date must be on or before 31 March "+YC.fyEndYear+".",sec:"who"});
