...
function expSi(j){
  ...
  /* 3a REMOVE (AY2025-26): ScheduleSI.EditAutopoulatedDetail dropped. */
  j.ScheduleSI=SI;
  ...
  if(firms.length){
    const arr=firms.map(f=>{const o={
      ...
      /* 3a REMOVE (AY2025-26): ScheduleIF PartnerFirmDetails[] IntrstAmtDueOrRecv / RemunernAmtDueOrRecv dropped. */
      return o;});
    const T=G.ifTot||{};
    j.ScheduleIF={
      PartnerFirmDetails:arr,
      TotalProfitShareAmt:sg(T.profit),      /* required (signed) */
      /* 3a REMOVE (AY2025-26): ScheduleIF TotalIntrstAmtDueOrRecv / TotalRemunernAmtDueOrRecv dropped. */
