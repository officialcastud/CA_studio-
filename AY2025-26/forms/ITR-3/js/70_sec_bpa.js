...
function expBpa(j){
  ...
  /* 3a REMOVE (AY2025-26): PARTA_PL CreditsToPL.OthIncome.AmtofInterest / AmtofRem and the
     whole NonResidentPL / NonResidentPLDetails blocks are dropped from the AY2025-26 schema. */
  if(j.PARTA_PL){
    const oi=j.PARTA_PL.CreditsToPL&&j.PARTA_PL.CreditsToPL.OthIncome;
    if(oi){delete oi.AmtofInterest;delete oi.AmtofRem;}
    delete j.PARTA_PL.NonResidentPL; delete j.PARTA_PL.NonResidentPLDetails;
  }
  ...
  if(_bpaHas(B.trd)){ j.TradingAccount=_bpaNode(B.trd);
    /* 3a REMOVE (AY2025-26): TradingAccount IncomeFutureTrd / TurnoverFutureTrd dropped. */
    delete j.TradingAccount.IncomeFutureTrd; delete j.TradingAccount.TurnoverFutureTrd; }
