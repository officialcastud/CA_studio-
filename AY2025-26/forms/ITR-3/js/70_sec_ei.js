...
function expEi(j){
  ...
  /* line 3 — other exempt income; write rows carrying an amount.
     3d CHANGE (AY2025-26): the AY2026-27 Category/SubCategory/Description trio is
     replaced by OthersIncDtls[].{NatureDesc (10(x) enum, OTH otherwise), OthNatOfInc
     (free text), OthAmount}, and the block carries a dedicated exempt-dividend line
     (NatureofDescDivName "Dividend" + OthDividendAmt), both schema-required. */
  const others = (C.others||[]).filter(r=>r.amt || st0(r.cat) || st0(r.sub) || st0(r.desc));
  const exDiv = n0(C.exemptDiv);
  if(others.length || exDiv){
    o.OthersInc = { NatureofDescDivName:"Dividend", OthDividendAmt: exDiv,
      OthersIncDtls: others.map(r=>{
      const d = {};
      d.NatureDesc = sv(r.sub) || "OTH";
      put(d,"OthNatOfInc", sv(r.desc) ? sv(r.desc).slice(0,50) : undefined);
...
function impEi(I3){
  ...
  E.others = others.map(r=>({
    cat:"", sub:r.NatureDesc||"", desc:r.OthNatOfInc||"", amt:nz(r.OthAmount)   /* 3d: NatureDesc/OthNatOfInc */
  }));
  E.exemptDiv = nz((EI.OthersInc||{}).OthDividendAmt);
