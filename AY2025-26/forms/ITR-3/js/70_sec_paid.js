...
function engPaid(){
  ...
  /* Schedule IT (17A) — advance vs self-assessment split by deposit date.
     ...
     i.e. a challan on or before 31 Mar of the P.Y. end (YREND, 2025) is advance tax [T31=SUMIF(<2)],
     one on or after 1 Apr of the A.Y. (2025) is self-assessment tax [R31=SUMIF(>=2)].
...
function secPaid(){
  ...
  h+=note("A challan dated <b>on or before 31 March "+YC.fyEndYear+"</b> is advance tax (→ 10a of Part B-TTI); one dated <b>on or after 1 April "+YC.fyEndYear+"</b> is self-assessment tax (→ 10d). The date also places each advance challan in its 234C instalment. There is no dropdown — the split is by date.");
  h+=grid("paid.it",[
  ...
  h+=row("10a · Advance tax — challans on or before 31 Mar "+YC.fyEndYear,cell(P.adv));
  h+=row("10b · TDS — TDS 1 col 5 + TDS 2 col 9 + TDS 3 col 9",cell(P.tds));
  h+=row("10c · TCS — Schedule TCS col 7(i)",cell(P.tcs));
  h+=row("10d · Self-assessment tax — challans on or after 1 Apr "+YC.fyEndYear,cell(P.sat));
