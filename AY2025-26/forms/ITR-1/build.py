...
Outputs (kept in sync so gates and double-click both work):
  ...
  - Yukti_ITR1.html single-file equivalent: css inlined in <style>, all js in one
                    <script> block, in the identical load order. This is the
                    "single-file equivalent" the verification bar compares against
                    and the shape tools/gates/gate.py parses. It is a generated
                    artifact; edit js/ + css/, never this file.
...
def build():
    ...
    open(os.path.join(HERE, "Yukti_ITR1.html"), "w", encoding="utf-8").write(single)

    print("built index.html (%d scripts) and Yukti_ITR1.html from %d js files" % (len(files), len(files)))
