...
SPECIAL: the ITR-2 base was a single hand-assembled HTML file (one inline
<script>), not src/*.js parts. Step 1 partitioned that single <script> into the
js/ files below, split ONLY at safe top-level boundaries (a line at column 0 that
begins a new function / const / /* ==== */ banner — never mid-function). Files
50/60/70/95 each carry a leading `"use strict";` so every classic <script src>
runs strict exactly as the original single strict script did.
...
Outputs (kept in sync so gates and double-click both work):
  ...
  - Yukti_ITR2.html single-file equivalent: css inlined in <style>, all js in one
                    <script>, in the identical load order.

Load order = sorted(js/*.js). The trailing boot file (95_boot.js) sorts last and
must stay last: it wires the export button and calls paint() once every module is
defined.
...
def build():
    ...
    open(os.path.join(HERE, "Yukti_ITR2.html"), "w", encoding="utf-8").write(single)

    print("built index.html (%d scripts) and Yukti_ITR2.html from %d js files" % (len(files), len(files)))
