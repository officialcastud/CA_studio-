...
/* AY2025-26 YEAR OVERLAY: AMTC_YRS is reassigned from js/05_year_config.js to the 12 assessment
   years 2013-14 .. 2024-25 (the AY2025-26 ScheduleAMTC enum; maxItems 13 -> 12). */
let AMTC_YRS=["2013-14", "2014-15", "2015-16", "2016-17", "2017-18", "2018-19", "2019-20", "2020-21", "2021-22", "2022-23", "2023-24", "2024-25", "2025-26"];
...
/* AY2025-26 YEAR OVERLAY: SLAB_NEW is reassigned from YC.slabNew in js/05_year_config.js
   (kept a `let` so the overlay file, which loads after this one, can set the FY2024-25 table). */
let SLAB_NEW=[[400000,0],[800000,5],[1200000,10],[1600000,15],[2000000,20],
...
const CUT=new Date(2024,6,23);   /* Budget-2024 indexation cutoff (23-Jul-2024) — unchanged in FY2024-25 */
let DUE=new Date(2026,6,31), YREND=new Date(2026,2,31);   /* AY2025-26 YEAR OVERLAY: reassigned from YC in js/05_year_config.js */
...
/* ==================================================================
   ...
   ================================================================== */
const S={
  meta:{app:"yukti",form:"ITR-2",ay:"2025-26",ver:1},
  pi:{status:"I",res:"RES",empcat:"OTH",addr2same:"Yes",country:"91",fpi:"No",rep:"No",dir:"No",partner:"No",unl:"No",s5a:"No",s115h:"No",pe:"No"},
  decl:{flag:"No"},
  ...
  cg:null,
  os2:null,
  via:{}, g80:[], gga:[], ggc:[], ra:[], c80c:[], pen80ccc:[], pen80ccd1:[], pen80ccd1b:[],
  d80:{selfSr:"N",parSr:"N"}, u80:{}, dd80:{}, e80:{e:[],ee:[],eea:[],eeb:[]},
  loss:{cfl:{},editC:"",editB:""},
  ei:[], ei2:{}, spi:[], trp:{}, pti2:[], fsi2:[], fa2:{}, tr2:{}, amtc:{}, esop:{yrs:{}}, sch5a2:{h:{}}, al2:{},
  fsi:{on:false,rows:[]}, tr:{on:false,rows:[]}, fa:{on:false,bank:[],custodial:[],
       equity:[],immovable:[],other:[]},
  al:{on:false},
  sch5a:{on:false}, pti:{on:false,rows:[]}, esop:{on:false,rows:[]},
  tax:{}, amt:{},
  tds1:[], tds2:[], tds3:[], tcs:[], it:[],
  bank:[], ver:{cap:"S"},
  open:{}, C:{}
};
...
function engCG(){
  ...
  [["a6",C.a6],["b3i",C.b3i],["b3ii",C.b3ii],["b3iii",C.b3iii],["b9",C.b9],["b6i",C.b6i],["b6ii",C.b6ii],["b6iii",C.b6iii]]
...
/* ==================================================================
   ...
   ================================================================== */
/* AY2025-26 YEAR OVERLAY: CFL_YEARS is reassigned from js/05_year_config.js to shift the
   eight carry-forward FY labels back one year (2017-18 .. 2024-25); the relative schema keys
   (Prev8th .. PrevYrToAY) are fixed. Kept a `let` so the overlay file can set the year window. */
let CFL_YEARS=[["2018-19","LossCFFromPrev8thYearFromAY",false],["2019-20","LossCFFromPrev7thYearFromAY",false],
...
function engTax(){
  ...
  if(S.pi.status==="I"&&S.pi.res!=="NRI"){
    if(isNew()){const RTI=YC.rebateNewTI,RMX=YC.rebateNewMax;if(ti<=RTI)rebate=Math.min(taxOn,RMX);   /* AY2025-26: §87A new — TI<=7L -> up to 25,000, marginal above */
      else{const slabOnly=normalTax;const excess=ti-RTI;if(slabOnly>excess){marginal=slabOnly-excess;rebate=Math.min(taxOn,marginal);}}}
    else if(ti<=YC.rebateOldTI)rebate=Math.min(taxOn-SI.tax112A-SI.tax115AD,YC.rebateOldMax);}   /* OLD: TI<=5L -> up to 12,500 (unchanged) */
...
let Q_CUT=[new Date(2025,5,16),new Date(2025,8,20),new Date(2025,11,15),new Date(2026,2,16)];   /* AY2025-26 YEAR OVERLAY: reassigned from YC in js/05_year_config.js */
...
function engInt(){
  ...
  const matchedSAT=satIn(new Date(YC.ayStartYear,3,1),dueDate);   /* AY2025-26: 1-Apr-2025 (AY start) */
  ...
  if(assessed>=10000&&net>=10000&&P.adv<0.9*assessed){
    ...
    const end=filed||new Date();const months=Math.min(24,monthsBetween(new Date(YC.ayStartYear,3,1),end)||1);   /* AY2025-26: 234B runs from 1-Apr-2025 */
    for(let m=0;m<months;m++){const mStart=new Date(YC.ayStartYear,3+m,1),mEnd=new Date(YC.ayStartYear,4+m,0);
...
function engChecks(){
  ...
  if(CG.on){
    (S.cg.land||[]).forEach((p,n)=>{const r=p._||engLand(p);const lbl="Property "+(n+1);
      ...
      if(D(p.sale)&&(D(p.sale)>YREND||D(p.sale)<new Date(YC.fyStartYear,3,1)))   /* AY2025-26: sale must fall in FY2024-25 (1-Apr-2024 .. 31-Mar-2025) */
