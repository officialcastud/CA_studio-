...
Outputs (kept in sync so gates and double-click both work):
  ...
  - Yukti_ITR4.html single-file equivalent: css inlined in <style>, all js in one
...
def build():
    ...
    open(os.path.join(HERE, "Yukti_ITR4.html"), "w", encoding="utf-8").write(single)

    print("built index.html (%d scripts) and Yukti_ITR4.html from %d js files" % (len(files), len(files)))
