# Yukti sheet-reader task (ITR-3, Phase 3) — shared instructions
...
## Read, in this order
1. `python3 tools/dump.py --form ITR-3 --sheet "<SHEET>"` — every row. **A row printed with `H` right after its number is HIDDEN** → it goes only under "Hidden rows — not built", never as an item.
2. `python3 tools/dump.py --form ITR-3 --sheet "<SHEET>" --formulas` — the formulas. Every formula carrying a rule (a cap, MIN/MAX, a condition, a cross-sheet reference) becomes a line in "The rules the sheet computes", **with its cell reference**.
3. `python3 tools/dump.py --form ITR-3 --dropdowns "<SHEET>"` — every dropdown; list every value.
4. For each schema block: `python3 tools/dump.py --form ITR-3 --schema <Block>` and `python3 tools/dump.py --form ITR-3 --leaves <Block>`. **Every schema leaf key name must appear verbatim in the book**, beside the sheet item it maps to.
5. `grep -n "<distinctive phrase>" sources/ITR-3/vba_text.txt` when a validator / auto-populate / month-count rule is hinted.
6. `grep -in "<schedule name>" books/ITR-3/rules.json` — the item numbering used by the RULES document is the numbering the book uses.

If given an ITR-2 model book, read it as a **style model only** — never copy ITR-3 figures, item numbers, or rules from it; ITR-3 content comes solely from ITR-3's own sources.
