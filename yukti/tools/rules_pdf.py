...
def main():
    ...
    doc=fitz.open(pick[0]);rules=[];cat=None;text_x=None
    for pno in range(len(doc)):
        ...
        # auto-detect the scenario-text column x (layout varies by form; a right/centre-aligned Sl.No.
        # column makes 1-2 digit serials start further right than 3-digit ones, so a fixed x<110 drops them).
        # text_x = the mode of first-word x0 over continuation lines (first word not a serial token / header).
        from collections import Counter
        _tx=Counter()
        for y in ys:
            if ann_y is not None and y>=ann_y:continue
            _w0=sorted(lines[y],key=lambda w:w[0])
            if _w0 and not re.fullmatch(r"\d{1,3}\.?",_w0[0][4]) and not re.match(r"^(Scenario|Sl\.|Category|Table|ITR|Version|Directorate|CBDT_|Annexure|Page)",_w0[0][4]):_tx[round(_w0[0][0])]+=1
        if _tx:text_x=_tx.most_common(1)[0][0]
        tx=text_x if text_x is not None else 110
        for y in ys:
            if ann_y is not None and y>=ann_y:continue
            ws=sorted(lines[y],key=lambda w:w[0])
            if ws and re.fullmatch(r"\d{1,3}\.?",ws[0][4]) and ws[0][0]<tx-3:nums.append((y,int(ws[0][4].rstrip("."))))
        if not nums:continue
        bounds=[((nums[i-1][0]+y)/2 if i>0 else y-15,(y+nums[i+1][0])/2 if i+1<len(nums) else 9999,n,y) for i,(y,n) in enumerate(nums)]
        page_rules={}
        for y in ys:
            if ann_y is not None and y>=ann_y:continue
            ws=[w for w in sorted(lines[y],key=lambda w:w[0]) if w[0]>=tx-3]
            if not ws:continue
