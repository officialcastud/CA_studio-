/* =====================================================================
   ITR-7 · Section "cg" — Schedule CG (Capital Gains).
   Built ONLY from books/ITR-7/Schedule_CG.md (block ScheduleCG),
   books/ITR-7/{enums.json,section_map.json,skeleton.json} and the schema
   sources/ITR-7/ITR-7_2026_Main_V0_1_schema.json for exact keys.

   Structural template: forms/ITR-6/src/70_sec_cg.js (the same schedule) —
   but every field, number, formula, schema key and enum here is taken from
   the ITR-7 book (the trust/institution return), never from ITR-6.

   Schema block (section_map → cg): ScheduleCG only
   ...
   ITR-7 vs ITR-6 — the substantive differences taken from the book/schema:
   - There is NO Schedule 112A / Schedule 115AD scrip schedule and NO Schedule
     VDA block on the "cg" section.  B4 (112A) and B7 (115AD proviso) are single
     amounts:  B4 = LongTermCapGain.SaleOfEquityShareUs112A.SaleOfEquityShareUs112AAmt,
     B7 = LongTermCapGain.NRISaleOfEquityShareUs112A.NRISaleOfEquityShareUs112AAmt.
   - LTCG land Cost-of-Improvement is NESTED —
     ...
     — whereas the STCG land block keeps a flat ImproveCost.  (Book §10.)
   - Schedule VDA is a SEPARATE section ("vda").  C2 (IncmFromVDATrnsf), the
     Part-F VDA quarter row and the Schedule-SI 115BBH(ii) capital-gain feed are
     built here from the VDA capital-gain total the vda section publishes on
     S.C.vda.  This section owns none of Schedule VDA's own keys.
   - Land heads A1/B1 carry 54G/54GA (A1) and 54D/54EC/54G/54GA (B1); A6 =
     54G/54GA; B8 = 54D/54G/54GA; B2/B3 slump/listed as ITR-6.  Enum spellings
     are the ITR-7 schema's own (Appendix 1 / schema).
   - Live Table-E buckets (book §6, rows 474-485): short {20/30/app/DTAA},
   ...
   - Non-resident heads A4/A5/A9, B5/B6/B7/B11 are surfaced only for a
     non-resident (residential status S.pi.res); hidden for a resident body.
...
/* ---- dropdown tables (from the ITR-7 book / schema, verbatim) ------ */
const EXST_LAND=[["54G","Sec 54G"],["54GA","Sec 54GA"]];                          /* A1 land di/dii */
const EXLT_LAND=[["54D","Sec 54D"],["54EC","Sec 54EC"],["54G","Sec 54G"],["54GA","Sec 54GA"]]; /* B1 land di-div */
const EXA6=[["54G","Sec 54G"],["54GA","Sec 54GA"]];                               /* A6 fi/fii */
const EXB8=[["54D","Sec 54D"],["54G","Sec 54G"],["54GA","Sec 54GA"]];             /* B8 di/dii/diii */
const BBST=[["STL20","Loss from buy back of 'shares taxable at 20%'"],
  ["STL30","Loss from buy back of 'shares taxable at 30%'"],
  ["STLAR","Loss from buy back of 'shares taxable at applicable rate'"]];         /* A(A) Rate enum */
const TRCYN=[["Y","Yes"],["N","No"]];
const UNUTFLAG=[["N","No"],["Y","Yes"],["X","Not Applicable"]];                   /* UnutilizedStcg/LtcgFlag Y/N/X */
/* A7a / B9a unutilised-CGAS deemed-income tables */
const DEEM_ST_SEC=[["54G","Sec 54G"],["54GA","Sec 54GA"]];                        /* A7a SectionClmd */
const DEEM_LT_SEC=[["54D","Sec 54D"],["54G","Sec 54G"],["54GA","Sec 54GA"]];      /* B9a SectionClmd */
const DEEM_PY=[["2022-23","2022-23"],["2023-24","2023-24"],["2024-25","2024-25"]]; /* PrvYrInWhichAsstTrnsfrd */
const DEEM_YRACQ=[["2022","2022-23"],["2023","2023-24"],["2024","2024-25"],["2025","2025-26"]]; /* YrInWhichAssetAcq */
/* A9 / B11 DTAA "Item No. ... in which included" dropdowns (schema ItemNoincl) */
const ITEM_STCG=[["A1e","A1e"],["A2c","A2c"],["A3ie","A3ie"],["A3iie","A3iie"],["A4a","A4a"],
  ["A4b","A4b"],["A5e","A5e"],["A6g","A6g"],["A7","A7"],["A8a","A8a"],["A8b","A8b"],["A8c","A8c"]];
const ITEM_LTCG=[["B1g_12.5%","B1g @12.5%"],["B2e_12.5%","B2e @12.5%"],["B3c","B3c"],["B4_12.5%","B4 @12.5%"],
  ["B5_12.5%","B5 @12.5%"],["B6ic_12.5%","B6ic @12.5%"],["B6iic_12.5%","B6iic @12.5%"],
  ["B6iiic_12.5%","B6iiic @12.5%"],["B6ivc_12.5%","B6ivc @12.5%"],["B7_12.5%","B7 @12.5%"],
  ["B8e_12.5%","B8e @12.5%"],["B9_12.5%","B9 @12.5%"],["B10(a1)_12.5%","B10(a1) @12.5%"],
  ["B10(a2)_12.5%","B10(a2) @12.5%"]];
/* StateCode list for the buyer detail (schema "State", 38 values — Appendix 1) */
const CG_STATE=[["01","Andaman and Nicobar Islands"],["02","Andhra Pradesh"],["03","Arunachal Pradesh"],
  ...
  ["25","Puducherry"],["26","Punjab"],["27","Rajasthan"],["28","Sikkim"],["29","Tamilnadu"],["30","Tripura"],
  ["31","Uttar Pradesh"],["32","West Bengal"],["33","Chhatishgarh"],["34","Uttarakhand"],["35","Jharkhand"],
...
/* Part D · DeducClaimInfo entry tables (schema DeducClaimInfo).
   Live ITR-7 sections: 54D (acq/land-building), 54EC (bonds), 54G, 54GA.
   `invest` = the 54EC shape; `acq` = uses DateofAcquisition (54D). */
const DCLAIM=[
...
/* the five 234C date-ranges (CG book Part F, rows 489-501) */
...
/* resident body unless the return's residential status says otherwise (S.pi.res) */
function isNr(){return st0((S.pi||{}).res||"RES")!=="RES";}

/* the VDA capital-gain total, published by the separate "vda" section on
   S.C.vda.  Its exact key isn't owned here, so read defensively (>=0). */
function vdaCG(){const v=(S.C&&S.C.vda)||{};
  const cands=["capGain","cg","C2","capgain","CG","TotIncCapGain","total"];
  for(let i=0;i<cands.length;i++){const x=v[cands[i]];
    if(x!=null&&x!==""&&isFinite(N(x)))return Math.max(0,R(N(x)));}
  return 0;}
...
/* one land/building property — A1 (STCG) or B1 (LTCG); a trust gets NO
...
/* an aggregate head carrying the section-48 block (A3i, A3ii, A5, A6, B3, B8, NR B6) */
...
/* A9 / B11 DTAA relief: per row the applicable rate is the lower of the treaty
   rate and the I.T.-Act rate; a row the treaty makes fully exempt (applicable
   rate 0) is "not chargeable to tax", the rest is taxable at the special DTAA
   rate. */
...
function engCg(){
  ...
  const vcg=vdaCG();
  if(!cgOn()){
    S.C.cg={on:false,nri:nri,A:{total:0},B:{total:0},C1:0,C2:vcg,C3:vcg,
      gain:Z(),loss:Z(),used:Z(),absorbed:Z(),after:Z(),totSet:Z(),remain:Z(),matrix:{},F:{},
      vda:{cg:vcg},buckets:{},siFeed:vcg>0?{"5BBHii":vcg}:{},dedD:{},dedTotal:0,cflSTCL:0,cflLTCL:0,
      shortTerm:0,longTerm:0,total:vcg,income:vcg};
    return;
  }
  ...
  B.b4 = {a:R(N((C.b4||{}).amt)), gain:R(N((C.b4||{}).amt))};        /* B4 = SaleOfEquityShareUs112AAmt */
  {const b5g=N((C.b5||{}).gain);
    B.b5={without:R(b5g), gain:R(b5g)};}                   /* B5 (NR) LTCG without indexation */
  B.b6 = {rows:(C.b6||[]).map(r=>engAgg(r,{unq:1}))};       /* B6 (NR) */
  B.b6.gain=R(B.b6.rows.reduce((a,x)=>a+x.gain,0));
  B.b7 = nri?{a:R(N((C.b7||{}).amt)), gain:R(N((C.b7||{}).amt))}:{a:0,gain:0};   /* B7 = NRISaleOfEquityShareUs112AAmt (NR) */
  ...
  /* Composition (CG book §6, live Table E rows 474-485):
  ...
  if(C.editE && C.Eover){                                   /* row 486 "Do you want to edit" override */
  ...
  /* ---- Part C · summary ------------------------------------------- */
  const C2=vcg;                                             /* C2 = Sch VDA item B (from the vda section) */
  ...
  /* ---- special-rate carve for 112A / 115AD-proviso (Schedule SI) --- */
  ...
  const si112a115ad=Math.max(0,Math.min(Math.max(0,after.lt125-si112a),base115ad)); /* 115AD proviso @12.5% */
  const si112=Math.max(0,after.lt125-si112a-si112a115ad);            /* other LTCG @12.5% */
  const buckets={
    si111a20:after.st20, si30:after.st30, stApp:after.stApp, stDTAA:after.stDTAA,
    si112a:si112a, si112a115ad:si112a115ad, si112:si112, ltDTAA:after.ltDTAA, si115bbh:C2};

  /* ================= Schedule-SI feed (special-rate CG heads) =================
     The si section consumes S.C.cg.siFeed keyed by the exact ITR-7 Schedule-SI
     SecCode (SI.md §2/§3/§6).  Value = a plain number (income; si.js supplies
     the rate and computes the tax) OR, for a DTAA head, an {inc,tax} object
     carrying the treaty-rate tax.  Every figure is REUSED from the ladder above
     (Part A/B gains, Table-E after-set-off slots and the ₹1.25L LTCG split).

     A special-rate head shares its Table-E rate-slot with siblings, so the
     post-set-off slot total (after.*) is apportioned back to its member heads by
     their gross gain (splitSI).  Applicable-/normal-rate slots (stApp: A1/A2/A4b/
     A6/A7/A8-app) carry NO special rate and are deliberately excluded.
        1A       111A / NR-111A @20%                <- A3(i)e + A4a  (E.st20)
        5AD1biiP 115AD(1)(b)(ii) STCG by FII @20%   <- A3(ii)e       (E.st20)
        PTI_STCG20P pass-through STCG @20%          <- A8a           (E.st20)
        5ADii    115AD(1)(ii) STCG @30%             <- A5e           (E.st30)
        PTI_STCG30P pass-through STCG @30%          <- A8b           (E.st30)
        2A       112A LTCG @12.5%                   <- B4            (si112a)
        5ADiiiP  115AD(1)(b)(iii) proviso LTCG      <- B7            (si112a115ad)
        21       112 LTCG (land/slump/B5/B8/B9)     <- B1g/B2e/B5/B8e/B9 (si112)
        22       112(1) listed sec/ZCB @12.5%       <- B3c           (si112)
        21ciii/5AB1b/5AC1c/5ADiii  NR 112(1)(c)/115AB/115AC/115AD  <- B6 rows (si112)
        PTI_LTCG12_5P112A / PTI_LTCG12_5P pass-through LTCG @12.5% <- B10a1/B10a2 (si112)
        DTAASTCG / DTAALTCG  STCG/LTCG at the DTAA treaty rate <- A9b/B11b
        5BBHii   115BBH VDA capital-gain income @30% <- Schedule VDA item B (C2) */
  const splitSI=(dst,afterAmt,members)=>{
  ...
  /* STCG @30% slot (E.st30) — FII 115AD(1)(ii) other than 111A, pass-through */
  splitSI(siFeed,after.st30,{
    "5ADii":Math.max(0,A.a5.gain),
    "PTI_STCG30P":Math.max(0,N(a8.r30))});
  /* LTCG @12.5% — 112A (2A) and 115AD-proviso (5ADiiiP) carved out first
  ...
  /* DTAA heads — treaty-rate tax: {inc, tax}. inc = post-set-off amount;
     tax = inc × the applicable (lower of treaty/IT-Act) rate. */
  ...
  /* 115BBH VDA capital-gain income @30% (Schedule VDA item B = C2) — not part
     of Table E, taken directly from the vda section */
  if(C2>0)siFeed["5BBHii"]=R(C2);

  S.C.cg={on:true,nri:nri,land,A,B,E,gain,loss,used,absorbed,matrix,after,totSet,remain,F,Fauto,
    vda:{cg:C2},buckets,siFeed,dedD,dedTotal:R(dedTotal),cflSTCL,cflLTCL,
...
function landBlock(p,i,lt){const r=p._||engLand(p);let h="";
  ...
  h+=note("Furnishing of PAN is mandatory if tax was deducted u/s 194-IA or the PAN is quoted by the buyer.");
...
/* the section-48 aggregate body (shared by A3i/A3ii/A5/A6/B3/B8 and NR B6 rows) */
...
function quartersTable(G){
  ...
  KEYS.forEach(k=>{h+='<tr><td class="l">'+esc(LBL[k])+'</td>';
    ...
    h+='</tr>';});
  /* VDA @30% row (from the vda section; the whole capital-gain total, disclosure) */
  h+='<tr><td class="l">VDA @30%</td><td>'+cell(0)+'</td><td>'+cell(0)+'</td><td>'+cell(0)+'</td><td>'+cell(0)+'</td><td>'+cell(G.C2)+'</td></tr>';
  h+='</tbody></table></div>';
  return h;
}
...
function dtaaGrid(key,rows,itemOpts){
  return grid(key,[
    ...
    {h:"Item included",k:"itemno",t:"sel",opts:itemOpts},
  ...
  ],rows,{empty:"No DTAA relief entered.",add:"Add a DTAA row",min:"1420px"});
}
...
function secCg(){
  ...
  if(!G.on){
    return note("Turn this on to open Schedule CG — land/building, slump sale, listed equity (112A), other assets, deemed gains, pass-through, DTAA and buy-back losses. Virtual Digital Assets are entered in the separate Schedule VDA section.")+
  ...
  a+=aggBlock("cga3i","A3(i) · 111A equity share / EOMF / business trust (STT paid)","A3ie","cg.a3i",C.a3i,G.A.a3i,{loss94:1});
  ...
  a+=fold("cga7","A7","A7 · Amount deemed to be STCG",RS(G.A.a7.gain),
    row("Any unutilized capital gain of an earlier year now chargeable?",
      sel("cg.a7.unutFlag",UNUTFLAG,{blank:true}),{ref:"UnutilizedStcgFlag"})+
  ...
  /* B4 112A — single amount */
  b+=fold("cgb4","B4","B4 · Equity / EOMF / business-trust units u/s 112A (STT paid)",RS(G.B.b4.gain),
    note("Long-term capital gain on 112A equity/units (net of the section-112A working). Feeds Schedule SI @12.5%.")+
    row("LTCG on assets at B4 u/s 112A",inp("cg.b4.amt",{n:1}),{ref:"B4"}),{});
  ...
  b+=fold("cgb9","B9","B9 · Amount deemed to be LTCG",RS(G.B.b9.gain),
    row("Any unutilized capital gain of an earlier year now chargeable?",
      sel("cg.b9.unutFlag",UNUTFLAG,{blank:true}),{ref:"UnutilizedLtcgFlag"})+
  ...
  if(G.nri){
    ...
    let b6="";
    (C.b6||[]).forEach((rw,i)=>{const rr=(G.B.b6.rows||[])[i]||engAgg(rw,{unq:1});
      b6+=blk("cgb6_"+i,"B6 asset "+(i+1),RS(rr.gain),
        row("Section",inp("cg.b6."+i+".sec",{max:12,ph:"112(1)(c)/115AB/115AC/115AD"}),{ref:"SectionCode"})+
    ...
    b+=fold("cgb7","B7","B7 · FII/FPI 112A route u/s 115AD(1)(b)(iii) proviso",RS(G.B.b7.gain),
      note("Long-term capital gain on equity/units for a FII/FPI non-resident (115AD proviso). Feeds Schedule SI @12.5%.")+
      row("LTCG on assets at B7",inp("cg.b7.amt",{n:1}),{ref:"B7"}),{});
    b+=fold("cgb11","B11","B11 · LTCG not chargeable / chargeable at special rate per DTAA",RS(G.B.b11.special),
      dtaaGrid("cg.b11",C.b11||[],ITEM_LTCG)+
  ...
  if(G.C2)h+=note("C2 is the capital-gain total from the separate Schedule VDA section.");
  ...
  h+=fold("cgE","E","Set-off of current-year capital losses",RS(G.C1),
    setoffTable(G)+
    row("Edit the auto-populated set-off?",sel("cg.editE",[["false","No"],["true","Yes"]],{blank:false}),{ref:"row486"}),{});

  /* ---- F · quarterly split (234C) ---- */
  h+=fold("cgF","F","Accrual / receipt of capital gain (quarterly, for 234C)","—",
    quartersTable(G)+
    row("Edit the auto-filled quarters?",sel("cg.editF",[["false","No"],["true","Yes"]],{blank:false}),{ref:"row489"}),{});
...
/* =====================================================================
   ...
   ===================================================================== */
function expCg(j){
  const G=S.C.cg;if(!G||!G.on){
    /* even when off, C2/VDA-accrual are required if there is VDA capital gain;
       but Schedule CG as a whole is emitted only when the head is on.  When off
       there is nothing to write (Part B-TI/SI read S.C.cg scalars directly). */
    return;
  }
  ...
  const exList=(ded,pairs)=>{const ex=[];pairs.forEach(([k,c])=>{if(N((ded||{})[k]))ex.push({ExemptionSecCode:c,ExemptionAmount:n0(ded[k])});});return ex;};

  /* ---- Part A (ShortTermCapGain) ---- */
  const ST={};
  const stLand=(G.land.st||[]).map(p=>{const r=p._;const ex=exList(p.ded,[["s54G","54G"],["s54GA","54GA"]]);
  ...
  /* A3 EquityMFonSTT[] — (i) 1A 111A, (ii) 5AD1biip 115AD(1)(b)(ii) FII (schema enum) */
  const mf=[];
  ...
  if(N((C.a6||{}).unqCons)||N((C.a6||{}).othCons)||N((C.a6||{}).cost)||N(A.a6.dcg)){const ex6=exList((C.a6||{}).ded,[["s54G","54G"],["s54GA","54GA"]]);
    ST.SaleOnOtherAssets={FullValueConsdRecvUnqshr:n0(C.a6.unqCons),FairMrktValueUnqshr:n0(C.a6.unqFmv),
      ...
      ExemptionOrDednUs54:Object.assign({ExemptionGrandTotal:n0(A.a6.ded)},ex6.length?{ExemptionOrDednUs54Dtls:ex6}:{}),
  ...
  /* schema-required objects on every ShortTermCapGain (emit even when nil);
     dfl() preserves any data written above */
  ...
  const LT={};
  const ltLand=(G.land.lt||[]).map(p=>{const r=p._;const ex=exList(p.ded,[["s54D","54D"],["s54EC","54EC"],["s54G","54G"],["s54GA","54GA"]]);
    return {DateofPurchase:ISO(p.buy),DateofSale:ISO(p.sale),FullConsideration:n0(p.cons),
      ...
      CostOfImprovements:{ImproveCost:n0(p.improve)},                 /* ITR-7: nested improve leaf */
      ExpOnTrans:n0(p.exp),TotalDedn:n0(r.biv),Balance:sg(r.c),
  ...
  LT.SaleOfEquityShareUs112A={SaleOfEquityShareUs112AAmt:sg(B.b4.gain)};   /* B4 direct amount */
  if(N((C.b8||{}).unqCons)||N((C.b8||{}).othCons)||N((C.b8||{}).cost)){const ex8=exList((C.b8||{}).ded,[["s54D","54D"],["s54G","54G"],["s54GA","54GA"]]);
    LT.SaleofAssetNADtls={SaleofAssetNA:{FullValueConsdRecvUnqshr:n0(C.b8.unqCons),FairMrktValueUnqshr:n0(C.b8.unqFmv),
      ...
      BalanceCG:sg(B.b8.c),ExemptionOrDednUs54:Object.assign({ExemptionGrandTotal:n0(B.b8.ded)},ex8.length?{ExemptionOrDednUs54Dtls:ex8}:{}),
  ...
  if(G.nri){
    ...
    if(b6.length)LT.NRIOnSec112and115={NRIOnSec112and115Dtls:b6.map(r=>{const rr=engAgg(r,{unq:1});
      return {SectionCode:st0(r.sec),FullValueConsdRecvUnqshr:n0(r.unqCons),FairMrktValueUnqshr:n0(r.unqFmv),
        ...
        BalanceCG:sg(rr.gain)};})};
    if(N((C.b7||{}).amt))LT.NRISaleOfEquityShareUs112A={NRISaleOfEquityShareUs112AAmt:sg(B.b7.gain)};
  ...
  /* SaleOfEquityShareUs112A + NRISaleOfEquityShareUs112A are schema-REQUIRED on
     every LongTermCapGain -> emit unconditionally; NRIProvisoSec48 is NOT
     required -> stays NR-only */
  if(G.nri)dfl(LT,"NRIProvisoSec48",{BalanceCG:0});
  dfl(LT,"SaleOfEquityShareUs112A",{SaleOfEquityShareUs112AAmt:0});
  dfl(LT,"NRISaleOfEquityShareUs112A",{NRISaleOfEquityShareUs112AAmt:0});
  ...
  /* quarters scaled to the post-BFLA figure (Schedule BFLA, published as
     S.C.loss.afterB per bucket) — keeps the break-up totalling the
  ...
  /* VDA @30% — the vda section owns the transaction dates, so its capital-gain
     total is placed in the last quarter (accrual disclosure only). */
  {const dr={};QK.forEach(q=>dr[q]=0);if(G.C2>0)dr[QK[4]]=n0(G.C2);
    AF.VDATrnsfGainsUnder30Per={DateRange:dr};}
...
function impCg(I7){
  ...
  (g(LT,"SaleofLandBuild.SaleofLandBuildDtls")||[]).forEach(x=>{const d={};
    ...
    land.push(Object.assign({buy:dmy(x.DateofPurchase),sale:dmy(x.DateofSale),lt:"Long",
      cons:nz(x.FullConsideration),sdv:nz(x.PropertyValuation),cost:nz(x.AquisitCost),
      improve:nz(g(x,"CostOfImprovements.ImproveCost")),exp:nz(x.ExpOnTrans),ded:d},bback(x)));});
  ...
  if(a6&&(N(a6.FullConsideration)||N(a6.FullValueConsdRecvUnqshr)||N(a6.FullValueConsdOthUnqshr)||N(g(a6,"DeductSec48.AquisitCost"))||N(a6.DeemedSTCGDeprAsset))){const d={};
    (g(a6,"ExemptionOrDednUs54.ExemptionOrDednUs54Dtls")||[]).forEach(e=>d["s"+e.ExemptionSecCode]=nz(e.ExemptionAmount));
  ...
  if(N(g(LT,"SaleOfEquityShareUs112A.SaleOfEquityShareUs112AAmt")))
    S.cg.b4={amt:nz(g(LT,"SaleOfEquityShareUs112A.SaleOfEquityShareUs112AAmt"))};
  ...
  if(N(g(LT,"NRISaleOfEquityShareUs112A.NRISaleOfEquityShareUs112AAmt")))
    S.cg.b7={amt:nz(g(LT,"NRISaleOfEquityShareUs112A.NRISaleOfEquityShareUs112AAmt"))};
  ...
  if(st0(CGb.EditAutopoulatedDetail))S.cg.editE=true;
...
function chkCg(){
  ...
  /* NR-only heads entered by a resident */
  if(!G.nri){
    if(N((C.a4||{}).sttPaid)||N((C.a4||{}).sttNot)||N((C.a5||{}).cost)||N((C.b5||{}).gain)||N((C.b7||{}).amt)||(C.b6||[]).length||(C.a9||[]).length||(C.b11||[]).length)
      out.push({lvl:"warn",t:"Non-resident capital-gains heads",
        m:"Heads A4/A5/A9 and B5/B6/B7/B11 apply only to a non-resident; the residential status is resident.",sec:"cg"});
  }
  /* VDA: dates not after 31 March of the FY — validated in the vda section */
...
reg({id:"cg", t:"Capital gains", ref:"Schedule CG",
  f:secCg, s:()=>!cgOn()?"None":((S.C.cg&&S.C.cg.C3)?RS(S.C.cg.C3):"Reporting"),
  eng:engCg, exp:expCg, imp:impCg, chk:chkCg, order:50, corder:45});
