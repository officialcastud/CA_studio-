/* =====================================================================
   ITR-7 · Section "os" — Income from Other Sources (trust / institution)
   Book: books/ITR-7/Schedule_OS.md   ·   Schema block: ScheduleOS
   Structural template: forms/ITR-6/src/70_sec_os.js — the shape (nine
   items + a quarterly matrix) is the same, but every field, number,
   formula, enum and schema key below is taken from the ITR-7 book and the
   ITR-7 schema (ScheduleOS), NOT ported from ITR-6.  Compute order: 44.

   HOW ITR-7's OS DIFFERS FROM ITR-6's (book §2, §3, §9)
   ---------------------------------------------------------------------
   · 1e "Any other income" carries TWO fixed lines, not one: the ITR-7-only
     row 24 "Income due to disallowance of exemption under clauses of
     section 10" (IncDisallwnExmpUs10, REQUIRED) — a trust that loses an
     exemption under a clause of section 10 offers the disallowed income
     here — plus row 25 56(2)(xii) (SumRecdPrYrBusTRU562xii, optional) and
     the free OthersInc.OthersIncDtls[] table.  1e is floored at nil.
   · The 2e DTAA table names its "Section of I.T. Act" column ItemNoincl
     (ITR-6 used SecITAct), and its disclosure total is
     TotalOSGrossChargblSplRate (ITR-6 used TotalAmtTaxUsDTAASchOs).  The
     ItemNoincl enum is the ITR-7 43-code list; NatureOfIncome is the
     9-value list {1ai,1aiii,1b,1c,1d,2ai,2aii,2c,2d}.  DTAA rows sit under
     IncChargblSplRateOS.NRIOsDTAA.NRIDTAADtlsSchOS[] (minItems 1 — the
     NRIOsDTAA node is emitted only when a row exists).
   · The 2c / 2d dropdowns are the ITR-7 special-rate section-code lists
     (18 codes each, incl. 5BBC "Anonymous donations u/s 115BBC" — a trust
     head; NO 115BBF/115BBG/115AB/specified-fund codes).
   · Item 10 has NINE live quarterly blocks (ITR-6 had ten): the
     specified-fund 115AD(1)(i)(B) dividend row is absent.

   ITEM 2 / ITEM 6 — the DTAA reclassification (same engine as ITR-6)
   ---------------------------------------------------------------------
   The DTAA attributable to the item-1 normal lines (natures 1ai,1aiii,1b,
   1c,1d), DTAA6, is MOVED from the normal leg to the special leg: item 6
   subtracts DTAA6 and item 2 adds it back.  The special-rate DTAA rows
   (2ai,2aii,2c,2d) already sit inside 2ai/2aii/2c/2d and are not re-added.
   TotalOSGrossChargblSplRate (the 2e disclosure total) sums ALL counted rows.

   CROSS-SECTION ROLLS published on S.C.os (consumed by cyla/si/tax/ti):
     total        — MAX(0,item6): normal-rate OS income → CYLA / Part B-TI
     loss         — a positive OS loss magnitude (item6<0) → CYLA
     netNormal / normal / six  — item 6 signed (aliases)
     ...
     dtaaTotal    — 2e TotalOSGrossChargblSplRate (→ SI DTAA / CYLA)
     income       — signed head figure (item2 + item6 + MAX0(8e))
     siFeed       — special-rate OS heads → Schedule SI (keyed by SecCode)
   ===================================================================== */
...
S.os = S.os || {
  ...
  /* 1e — any other income (two fixed lines + free table), floored at nil */
  disallow10:"",             /* row 24 — IncDisallwnExmpUs10 (required) */
  sum562xii:"",              /* row 25 — SumRecdPrYrBusTRU562xii */
...
/* ---- dropdowns (codes + labels verbatim from the ITR-7 ScheduleOS schema) ---- */
/* 2c — OthersGrossDtls[].SourceDescription (schema enum, 18 codes). The
   utility narrows this by residential status; the schema enum is the filed
   set and is shown here so any lawful value validates (book §3). */
const OS7_SPL = [
  ["5A1ai","115A(1)(a)(i)- Dividends received by non-resident (not being company) or foreign company chargeable u/s 115A(1)(a)(i) other than first proviso to section 115A(1)(a)(A)"],["5A1aA","115A(1)(a)(A)- Dividend received by non resident (Not being company) or foreign company from a unit in an International Financial Services Centre"],["5A1aii","115A(1)(a)(ii)- Interest received from govt/Indian Concerns received in Foreign Currency"],
  ["5A1aiia","115A(1) (a)(iia) -Interest from Infrastructure Debt Fund"],["5A1aiiaa","115A(1) (a)(iiaa) -Interest as per Sec. 194LC(1)"],["5A1aiiab","115A(1) (a)(iiab) -Interest as per Sec. 194LD"],
  ["5A1aiiac","115A(1)(a)(iiac)-Interest as per Sec. 194LBA"],["5A1aiii","115A(1) (a)(iii) - Income received in respect of units of UTI purchased in Foreign Currency"],["5A1bA","115A(1)(b)- Income from royalty or fees for technical services received from Government or Indian concern -chargeable u/s 115A(1)(b)"],
  ["5AC1ab","115AC(1)(a) - Income by way of interest on bonds purchased in foreign currency - non-resident"],["5AC1abD","115AC(1)(b) - Income by way of Dividend on GDRs purchased in foreign currency - non-resident"],["5AD1i","115AD(1)(i) -Income (other than Dividend) received by an FII in respect of securities (other than units as per Sec 115AB)"],
  ["5AD1iP","115AD(1)(i) -Income received by an FII in respect of bonds or government securities as per Sec 194LD"],["5AD1iDiv","115AD(1)(i) - Income (being dividend) received by an FII in respect of securities (other than units referred to in section 115AB)"],["5A1aiiaaP","115A(1) (a)(iiaa) -Interest as referred in proviso to section 194LC(1)"],
  ["5A1aiiaa2P","115A(1) (a)(iiaa) -Income received by non-resident as referred in second proviso to section 194LC(1)"],["5BBA","115BBA - Income received by non-residents sportsmen or sports associations"],["5BBC","115BBC - Anonymous Donations in certain cases chargeable u/s 115BBC"]];
/* 2d — PTIOthersGrossDtls[].SourceDescription (schema enum, 18 codes) */
const OS7_PTI = [
  ["PTI_5A1ai","PTI 115A(1)(a)(i)- Dividends received by non-resident (not being company) or foreign company chargeable u/s 115A(1)(a)(i) other than first proviso to section 115A(1)(a)(A)"],["PTI_5A1aA","PTI 115A(1)(a)(A)- Dividend received by non resident (Not being company) or foreign company from a unit in an International Financial Services Centre"],["PTI_5A1aii","PTI 115A(1)(a)(ii)- Interest received from govt/Indian Concerns received in Foreign Currency"],
  ["PTI_5A1aiia","PTI 115A(1) (a)(iia) -Interest from Infrastructure Debt Fund"],["PTI_5A1aiiaa","PTI 115A(1) (a)(iiaa) -Interest as per Sec. 194LC(1)"],["PTI_5A1aiiab","PTI 115A(1) (a)(iiab) -Interest as per Sec. 194LD"],
  ["PTI_5A1aiiac","PTI 115A(1)(a)(iiac)-Interest as per Sec. 194LBA"],["PTI_5A1aiii","PTI 115A(1) (a)(iii) - Income received in respect of units of UTI purchased in Foreign Currency"],["PTI_5A1bA","PTI 115A(1)(b)- Income from royalty or fees for technical services received from Government or Indian concern -chargeable u/s 115A(1)(b)"],
  ["PTI_5AC1ab","PTI 115AC(1)(a) - Income by way of interest on bonds purchased in foreign currency - non-resident"],["PTI_5AC1abD","PTI 115AC(1)(b) - Income by way of Dividend on GDRs purchased in foreign currency - non-resident"],["PTI_5AD1i","PTI 115AD(1)(i) -Income (other than Dividend) received by an FII in respect of securities (other than units as per Sec 115AB)"],
  ["PTI_5AD1iP","PTI 115AD(1)(i) -Income received by an FII in respect of bonds or government securities as per Sec 194LD"],["PTI_5BBA","PTI 115BBA - Income received by non-residents sportsmen or sports associations"],["PTI_5BBC","PTI 115BBC - Anonymous Donations in certain cases chargeable u/s 115BBC"],
  ["PTI_5AD1iDiv","PTI 115AD(1)(i) - Income (being dividend) received by an FII in respect of securities (other than units referred to in section 115AB)"],["PTI_5A1aiiaaP","PTI 115A(1) (a)(iiaa) -Interest as referred in proviso to section 194LC(1)"],["PTI_5A1aiiaa2P","PTI 115A(1) (a)(iiaa) -Income received by non-resident as referred in second proviso to section 194LC(1)"]];
/* 2e col (3) "Item No." (NRIDTAADtlsSchOS[].NatureOfIncome, 9 schema-enum values) */
const OS7_DTAA_NAT = [["1ai","1a(i) - Dividend"],["1aiii","1a(iii) - Dividend 2(22)(f)"],
...
/* 2e col (8) "Section of I.T. Act" (NRIDTAADtlsSchOS[].ItemNoincl, schema enum 43 codes) */
const OS7_DTAA_ITEMNO = [
  ["56i","56(2)(i)- Dividends"],["56","56(2)- Interest"],["56i_f","56(2)(i)- Dividends u/s 2(22)(f)"],
  ["562iii","56(2)(iii)-Rental income from machinery, plants, buildings etc."],["562x","56(2)(x) - Income under section 56(2)(x)"],["5A1ai","115A(1)(a)(i)- Dividends received by non-resident (not being company) or foreign company chargeable u/s 115A(1)(a)(i) other than first proviso to section 115A(1)(a)(A)"],
  ["5A1aA","115A(1)(a)(A) Dividend received by non resident from a unit in an International Financial Services Centre"],["5A1aii","115A(1)(a)(ii)- Interest received from govt/Indian Concerns recived in Foreign Currency"],["5A1aiia","115A(1) (a)(iia) -Interest from Infrastructure Debt Fund"],
  ["5A1aiiaa","115A(1) (a)(iiaa) -Interest as per Sec. 194LC"],["5A1aiiab","115A(1) (a)(iiab) -Interest as per Sec. 194LD"],["5A1aiiac","115A(1)(a)(iiac) -Interest as per Sec. 194LBA"],
  ["5A1aiii","115A(1) (a)(iii) - Income received in respect of units of UTI purchased in Foreign Currency"],["5A1bA","115A(1)(b)- Income from royalty or fees for technical services received from Government or Indian concern -chargeable u/s 115A(1)(b)"],["5AC1ab","115AC(1)(a) - Income by way of interest on bonds purchased in foreign currency - non-resident"],
  ["5AC1abD","115AC(1)(b) - Income by way of Dividend on GDRs purchased in foreign currency - non-resident"],["5AD1i","115AD(1)(i) -Income received by an FII in respect of securities (other than units as per Sec 115AB)"],["5AD1iP","115AD(1)(i) -Income received by an FII in respect of bonds or government securities as per Sec 194LD"],
  ["5BB","115BB - Income from winnings from lotteries, crossword puzzles etc"],["5BBA","115BBA - Income received by non-residents sportsmen or sports associations"],["5BBC","115BBC - Anonymous Donations in certain cases chargeable u/s 115BBC"],
  ["5BBJ","115BBJ - Income by way of winnings from Online games chargeable"],["PTI_5A1ai","PTI -Dividends received by non-resident (not being company) or foreign company chargeable u/s 115A(1)(a)(i) other than first proviso to section 115A(1)(a)(A)"],["PTI_5A1aA","PTI-115A(1)(a)(A)- PTI - Dividend received by non resident (Not being company) or foreign company from a unit in an International Financial Services Centre"],
  ["PTI_5A1aii","PTI - Interest received in the case of non-residents"],["PTI_5A1aiia","PTI - Interest received by non-resident from infrastructure debt fund"],["PTI_5A1aiiaa","PTI - Income received by non-resident as referred in section 194LC"],
  ["PTI_5A1aiiab","PTI - Income received by non-resident as referred in section 194LD"],["PTI_5A1aiiac","PTI - Income received by non-resident as referred in section 194LBA"],["PTI_5A1aiii","PTI - Income from units purchased in foreign currency in the case of non-residents"],
  ["PTI_5A1bA","PTI-115A(1)(b)Income from royalty or fees for technical services received from Government or Indian concern -chargeable u/s 115A(1)(b)"],["PTI_5AC1ab","PTI-115AC(1)(a) -Income by way of interest on bonds purchased in foreign currency - non-resident"],["PTI_5AC1abD","PTI-115AC(1)(b) - Income by way of Dividend on GDRs purchased in foreign currency - non-resident"],
  ["PTI_5AD1i","PTI-115AD(1)(i) -Income received by an FII in respect of securities (other than units as per Sec 115AB)"],["PTI_5AD1iP","PTI-115AD(1)(i) -Income received by an FII in respect of bonds or government securities as per Sec 194LD"],["PTI_5BBA","PTI-115BBA - Income received by non-residents sportsmen or sports associations"],
  ["PTI_5BBC","PTI - 115BBC - Anonymous Donations in certain cases chargeable u/s 115BBC"],["5AD1iDiv","115AD(1)(i) - Income (being dividend) received by an FII in respect of securities (other than units referred to in section 115AB)"],["5A1aiiaaP","115A(1) (a)(iiaa) -Interest referred to in section 194LC(1P) - chargeable u/s 115A(1)(a)(iiaa) @4 %"],
  ["5A1aiiaa2P","115A(1)(a)(iiaa)- Income received by non-resident as referred in second proviso to section 194LC(1)"],["PTI_5AD1iDiv","PTI-115AD(1)(i) - Income (being dividend) received by an FII in respect of securities (other than units referred to in section 115AB)"],["PTI_5A1aiiaaP","PTI_115A(1) (a)(iiaa) -Interest referred to in section 194LC(1P) - chargeable u/s 115A(1)(a)(iiaa) @4 %"],
  ["PTI_5A1aiiaa2P","PTI_115A(1)(a)(iiaa)- Income received by non-resident as referred in second proviso to section 194LC(1)"]];
/* 2e col (4) country dropdown (CountryCodeExcludingIndia, code:name — book Appendix, 249 codes) */
const OS7_CC = [
  ...
  ["9999","OTHERS"]];
const OS7_CCNAME = {}; OS7_CC.forEach(x=>{OS7_CCNAME[x[0]]=x[1];});
const OS7_TRC = [["Y","Yes"],["N","No"]];

/* item-10 quarterly rows (book §9): NINE live blocks, five DateRange leaves each
   (all standard periods), all leaves required. */
const OS7_QP = ["Upto15Of6","Up16Of6To15Of9","Up16Of9To15Of12","Up16Of12To15Of3","Up16Of3To31Of3"];
const OS7_QROWS = [
  {k:"lottery",     o:"IncFrmLottery",           l:"Winnings from lotteries, crossword puzzles, races, games, gambling, betting etc. referred to in section 2(24)(ix)  (= 2a(i))"},
  {k:"ongames",     o:"IncFrmOnGames",           l:"Winnings from Online games chargeable u/s 115BBJ  (= 2a(ii))"},
  ...
  {k:"d115a1ai",    o:"DividendIncUs115A1ai",     l:"Dividend Income u/s 115A(1)(a)(i) other than proviso to 115A(1)(a)(A) @20% (incl. PTI)"},
  {k:"d115a1aA",    o:"DividendIncUs115A1aA",     l:"Dividend income under proviso to 115A(1)(a)(A) @10% (incl. PTI) — IFSC unit"},
  {k:"d115ac",      o:"DividendIncUs115AC",       l:"Dividend Income u/s 115AC @10% (incl. PTI)"},
  {k:"d115ad1idiv", o:"DividendIncUs115AD1iDiv",  l:"Dividend Income (other than units u/s 115AB) received by a FII u/s 115AD(1)(i) @20% (incl. PTI)"},
  {k:"divdtaa",     o:"DividendDTAA",             l:"Dividend income taxable at DTAA rates"}
];
const OS7_QCOLH = ["Upto 15/6 (i)","16/6–15/9 (ii)","16/9–15/12 (iii)","16/12–15/3 (iv)","16/3–31/3 (v)"];

const os7_isNRI = ()=> (S.pi && S.pi.res) === "NRI";
...
function engOs(){
  ...
  /* 1e (J22) = MAX(0, IncDisallwnExmpUs10 + 56(2)(xii) + free rows) */
  const disallow10 = R(O.disallow10);                                     /* IncDisallwnExmpUs10 (required) */
  const biz562xii = R(O.sum562xii);                                       /* SumRecdPrYrBusTRU562xii */
  const othersSum = (O.others||[]).reduce((s,r)=>s+R(r.amt),0);
  const e1 = Math.max(0, disallow10 + biz562xii + othersSum);             /* AnyOtherIncome */
  const item1 = a1 + b1 + c1 + d1 + e1;                                   /* GrossIncChrgblTaxAtAppRate */
  ...
  /* 2e DTAA — for an NRI only TRC="Y" rows count; residents: all rows */
  const dtaaRows = (O.dtaa||[]).map(r=>{
    ...
    /* col 10 applicable rate = lower of treaty (col 6) and IT-Act (col 9); blank
       for a non-resident whose TRC is not "Y" */
    let appl = hasTreaty ? (hasItact ? Math.min(treaty,itact) : treaty) : (hasItact ? itact : "");
    if(os7_isNRI() && r.trc!=="Y") appl = "";
    const counts = os7_isNRI() ? (r.trc==="Y") : true;
  ...
  const f2 = dtaaRows.filter(r=>r.counts).reduce((s,r)=>s+r.amt,0);       /* TotalOSGrossChargblSplRate */
  ...
  /* 3b depreciation — allowed only if rental income (1c) offered, restricted to 1c */
  ...
  C.e1={disallow10:disallow10, biz562xii:biz562xii, others:othersSum, tot:e1};
  C.item1=item1;
  ...
  /* ---- rolls for other sections (cyla / si / tax / ti) ---- */
  C.total      = R(Math.max(0, item6));    /* normal-rate OS income → CYLA / Part B-TI */
  C.loss       = R(item6<0 ? -item6 : 0);  /* OS loss magnitude (item6<0) → CYLA */
  C.netNormal  = R(item6);                 /* signed net normal (alias) */
  ...
  C.raceHorse  = R(item8e);                /* signed; <0 → Schedule CFL */
  C.chargeable = R(item9);                 /* IncChargeableFrmOthSrc → Part B-TI */
  C.dtaaTotal  = R(f2);                    /* 2e total → Schedule SI DTAA / CYLA */
  ...
  /* ================= Schedule-SI feed (special-rate OS heads) =================
     70_sec_si.js consumes S.C.os.siFeed keyed by the schema SecCode (Schedule
     SI SplCodeRateTax). A value is a plain number (income — si.js supplies the
     rate and computes the tax) or {inc,tax} for the DTAA head (treaty tax).
     Every amount is REUSED from item 2 above — the 2a winnings, 2b 115BBE, and
     the 2c/2d dropdown incomes summed by their own section code (already the
     schema SecCode spelling) — never recomputed.

     DTAA netting: the 2e income is disclosed once, at the treaty rate, under the
     single OS-DTAA head (DTAAOS = the whole 2e total f2). Because the same DTAA
     income is also carried inside the 2a/2c/2d figures (the 2ai/2aii/2c/2d DTAA
     rows are NOT re-added into item 2, they already sit in those heads), each
     special-rate head is reduced by its own counted-DTAA portion so no rupee is
     taxed twice; the natures map 2ai→5BB, 2aii→5BBJ 1:1, and a 2c/2d row names
     its head in the ItemNoincl (enum) column. A head is emitted only when its
     remaining income is > 0. */
  const siFeed = {};
  const dtRows = (dtaaRows || []).filter(r => r.counts);
  ...
  dtRows.filter(r => r.nature === "2c" || r.nature === "2d")       /* 2c/2d named by ItemNoincl */
...
function secOs(){
  ...
  h += note('Income from Other Sources for a trust / institution. A trust that loses an exemption under a clause of section 10 offers the disallowed income at 1e (row 24). Dividend income reduced in Schedule BP must be offered here.');
  ...
  /* ===== item 1 — normal-rate income ===== */
  {
    ...
    b += row("From Deposit (Bank / Post Office / Co-operative)", inp("os.intDeposit",{n:1}), {ind:1, ref:"1b(ii)"});
    ...
    b += row("Income due to disallowance of exemption under clauses of section 10", inp("os.disallow10",{n:1}), {ind:1, ref:"1e",
      hint:"ITR-7 line: disallowed income where a clause-10 exemption is lost"});
  ...
  {
    let b = "";
    b += note("Nothing in item 3 (deductions u/s 57) may be set against the special-rate income at 2a–2e (sheet row 71).");
    ...
    b += grid("os.spl",[
      {k:"code",h:"Nature (special-rate section)",t:"sel",opts:OS7_SPL,req:1},
    ...
    b += grid("os.pti",[
      {k:"code",h:"Nature (PTI special-rate section)",t:"sel",opts:OS7_PTI,req:1},
      {k:"amt",h:"Amount",t:"num",w:"150px"}
    ...
    b += grid("os.dtaa",[
      ...
      {k:"nature",h:"Item No. in which included",t:"sel",opts:OS7_DTAA_NAT,req:1},
      {k:"cc",h:"Country",t:"sel",opts:OS7_CC,req:1},
      {k:"article",h:"Article of DTAA",t:"txt",max:16,req:1},
      {k:"treaty",h:"Rate as per Treaty (NIL if not chargeable)",t:"txt",max:10,req:1},
      {k:"trc",h:"TRC obtained",t:"sel",opts:OS7_TRC},
      {k:"sec",h:"Section of I.T. Act",t:"sel",opts:OS7_DTAA_ITEMNO,req:1},
  ...
  /* ===== item 3 — section 57 deductions ===== */
  {
    ...
    b += row("3b · Depreciation (available only if rental income offered at 1c)", (C.c1>0?inp("os.dDep",{n:1}):cell(0)), {ref:"3b",
      hint:(C.c1>0?"restricted to the 1c rental income":"available only when rental income is offered at 1c")});
  ...
  /* ===== items 4–7 ===== */
  {
    ...
    if(C.dtaa6) b += row("Less: DTAA amounts attributable to item-1 lines (moved to item 2)", cell(-(C.dtaa6||0)), {ind:1, ref:"6"});
    b += row("6 · Net income at normal rates (1 − DTAA − 3 + 4 + 5); a loss goes to CYLA", cell(C.item6), {ref:"6", cls:"tot"});
  ...
  /* ===== item 8 — race horses ===== */
  {
    ...
    if(R(RH.bal)<0) b += note("A negative 8e is a race-horse loss carried to Schedule CFL and taken as nil at item 9. It is set off against race-horse income only, for four years.");
  ...
  /* ===== item 10 — quarterly accrual/receipt ===== */
  {
    let t = '<div class="full"><table class="gt" style="min-width:960px"><thead><tr>'+
      '<th class="l">Other Source Income</th>'+OS7_QCOLH.map(x=>'<th>'+esc(x)+'</th>').join("")+'</tr></thead><tbody>';
    OS7_QROWS.forEach(qr=>{
      t += '<tr><td class="l">'+esc(qr.l)+'</td>'+
        OS7_QP.map(pk=>'<td>'+inp("os.q."+qr.k+"."+pk,{n:1})+'</td>').join("")+'</tr>';
    });
    t += '</tbody></table></div>';
...
function expOs(j){
  ...
  /* ---- item 3 Deductions object (book-required: Depreciation, TotDeductions) ---- */
  const D = C.ded||{};
  /* Expenses, Depreciation and TotDeductions are ALL schema-required leaves of
     the Deductions object (they must be present even at nil). */
  const ded = { Expenses:n0(D.expenses), Depreciation:n0(D.dep), TotDeductions:n0(D.tot) };
  ...
  const io = {
    ...
    RentFromMachPlantBldgs: n0(C.c1),
    Tot562x: n0(d1.tot),
    ...
    IncDisallwnExmpUs10: n0(e1.disallow10),
    AnyOtherIncome: n0(e1.tot),
    IncChargeableSpecialRates: n0(C.item2),
    LtryPzzlChrgblUs115BB: n0(C.s2ai),
    IncChrgblUs115BBE: n0(C.b2),
    CashCreditsUs68: n0(R(O.cc68)),
    UnExplndInvstmntsUs69: n0(R(O.ui69)),
    UnExplndMoneyUs69A: n0(R(O.um69a)),
    UnDsclsdInvstmntsUs69B: n0(R(O.udi69b)),
    UnExplndExpndtrUs69C: n0(R(O.ue69c)),
    AmtBrwdRepaidOnHundiUs69D: n0(R(O.hundi69d)),
    OthersGross: n0(C.d2c),
    PassThrIncOSChrgblSplRate: n0(C.d2d),
    IncChargblSplRateOS: { TotalOSGrossChargblSplRate: n0(C.f2) },
    Deductions: ded,
  ...
  /* 2e DTAA rows (emit NRIOsDTAA only when a row exists — minItems 1) */
  ...
  /* ---- item 8 race horses (root group; book-required Receipts/DeductSec57/Balance) ---- */
  const RH = C.horse||{};
  ...
  /* ---- item 10 quarterly DateRange objects (all nine blocks, always present) ---- */
  const q = O.q||{};
  OS7_QROWS.forEach(qr=>{
    const src = q[qr.k]||{};
    const dr = {};
    OS7_QP.forEach(pk=>{ dr[pk] = sg(R(src[pk])); });
    OS[qr.o] = { DateRange: dr };
  });

  /* Emit ScheduleOS only when the head actually carries income. A
     trust/institution with no other-source income files no ScheduleOS — an
     all-nil block would otherwise be an unlawful (and, via the required
     Deductions leaves, schema-valid but empty) disclosure. */
  const anyOS = [C.item1,C.item2,C.item6,C.item7,C.item9,C.f2,C.d2c,C.d2d,C.b2]
      .some(v=>n0(v)) || n0((C.horse||{}).receipts)
      || (O.others||[]).some(r=>st0(r.nat)||R(r.amt))
      || (O.spl||[]).some(r=>st0(r.code)||R(r.amt))
      || (O.pti||[]).some(r=>st0(r.code)||R(r.amt))
      || (C.dtaaRows||[]).length>0;
  if(anyOS) j.ScheduleOS = OS;
}

/* ---- import ------------------------------------------------------ */
function impOs(I7){
  const read = [];
  if(!I7 || !I7.ScheduleOS) return read;
  const OS = I7.ScheduleOS, io = OS.IncOthThanOwnRaceHorse||{};
  S.os = S.os || {};
  const O = S.os;
  ...
  O.disallow10 = nz(io.IncDisallwnExmpUs10);
  ...
  O.dtaa = ((((io.IncChargblSplRateOS||{}).NRIOsDTAA||{}).NRIDTAADtlsSchOS)||[]).map(r=>({
    ...
    trc:r.TaxRescertifiedFlag||"", sec:r.ItemNoincl||"",
  ...
  OS7_QROWS.forEach(qr=>{
    ...
    if(dr){ O.q[qr.k] = O.q[qr.k]||{}; OS7_QP.forEach(pk=>{ O.q[qr.k][pk] = nz(dr[pk]); }); }
  });
...
function chkOs(){
  ...
  if(R(O.dDep)>0 && !(C.c1>0))
    ...
    out.push({lvl:"warn", t:"Section 57 · depreciation", m:"Depreciation (3b) is restricted to the 1c rental income of "+RS(C.c1)+"; only that much is allowed.", sec:"os"});
  ...
  if(R(O.dIntClaimed)>0 && !(divForInt>0))
    ...
    out.push({lvl:"warn", t:"Section 57 · interest expenditure", m:"Interest expenditure u/s 57(1) is restricted to 20% of the dividend at 1a(i)+1a(ii); only "+RS(D.eligInt||0)+" of "+RS(R(O.dIntClaimed))+" claimed is eligible.", sec:"os"});

  /* 2e DTAA — NRI/TRC gate and applicable-rate note */
  (C.dtaaRows||[]).forEach((r,i)=>{
    if(os7_isNRI() && r.amt>0 && r.trc!=="Y")
  ...
  if((C.dtaaRows||[]).some(r=>r.amt>0) && !os7_isNRI())
  ...
  /* per-item DTAA caps: 1a(i), 1b, 1c, 1d, 2a(i), 2a(ii), 2c, 2d */
  const capMap = [
  ...
  capMap.forEach(cm=>{
    ...
    if(sum>R(cm.lim))
      out.push({lvl:"err", t:"DTAA vs "+cm.l, m:"The 2e DTAA amounts against "+cm.l+" ("+RS(sum)+") exceed that item's own figure "+RS(cm.lim)+".", sec:"os"});
  });
  ...
  if(R((C.horse||{}).bal)<0)
    out.push({lvl:"ok", t:"Race horses", m:"The race-horse balance (8e) is a loss of "+RS((C.horse||{}).bal)+"; it is carried to Schedule CFL and taken as nil at item 9.", sec:"os"});
...
/* ---- register ---------------------------------------------------- */
reg({id:"os", t:"Income from other sources", ref:"Schedule OS",
     f:secOs, s:()=>{const C=S.C.os||{}; return C.item9!=null?RS(C.item9):"";},
     eng:engOs, exp:expOs, imp:impOs, chk:chkOs, order:70, corder:44});
