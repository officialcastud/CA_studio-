    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <div className="stat-card">
          <span className="icon-badge icon-badge-sm mb-2"><BookOpen className="h-4 w-4" /></span>
        ...
        {/* Cash & Bank */}
        <div className={`stat-card ${cashBalance < 0 ? '!border-red-200' : ''}`}>
          <span className="icon-badge icon-badge-sm mb-2"><Wallet className="h-4 w-4" /></span>
          <p className="text-[10px] text-gray-400 uppercase tracking-wider mb-1">Cash & Bank</p>
          <p className={`text-2xl font-extrabold font-mono ${cashBalance < 0 ? 'text-red-600' : 'text-gray-900'}`}>
        ...
        {/* Net Profit / Loss */}
        <div className={`stat-card ${isProfit ? '' : '!border-red-200'}`}>
          <span className="icon-badge icon-badge-sm mb-2"><TrendingUp className="h-4 w-4" /></span>
          <p className="text-[10px] text-gray-400 uppercase tracking-wider mb-1">
            {isProfit ? 'Net Profit' : 'Net Loss'}
          </p>
          <p className={`text-2xl font-extrabold font-mono ${isProfit ? 'text-emerald-600' : 'text-red-600'}`}>
        ...
        {/* Accounts */}
        <div className="stat-card">
          <span className="icon-badge icon-badge-sm mb-2"><Scale className="h-4 w-4" /></span>
      ...
      {(totalDebtors > 0 || totalCreditors > 0) && (
        <div className="grid grid-cols-2 gap-3">
          <div className="stat-card">
            ...
            <p className="text-xl font-extrabold text-blue-600 font-mono">{shortINR(totalDebtors)}</p>
          </div>
          <div className="stat-card">
            <p className="text-[10px] text-gray-400 uppercase tracking-wider mb-1">Creditors (Payable)</p>
            <p className="text-xl font-extrabold text-amber-600 font-mono">{shortINR(totalCreditors)}</p>
          </div>
        </div>
      )}
