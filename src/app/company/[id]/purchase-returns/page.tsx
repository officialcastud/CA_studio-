...
export default function PurchaseReturnsPage() {
  ...
  const registerRows = buildInventoryRegisterRows(entries, 'DN');
  ...
  return (
    <div>
      <PageHeader title="Purchase Returns" description="Debit Notes issued">
        <div className="flex flex-col gap-2 items-end">
          <DateRangeFilter fromDate={fromDate} toDate={toDate} onDateChange={(f, t) => { setFromDate(f); setToDate(t); }} />
          <ExportButtons title="Purchase Returns" companyName={company.name} entityType={entityLabel} dateRange={`${fromDate} to ${toDate}`} columns={exportColumns} data={exportData} />
        </div>
      </PageHeader>
      {loading ? (
        ...
        <InventoryRegisterFormat
          ...
          data={registerRows}
          totals={totals}
          emptyMessage="No purchase return entries found."
          getJfHref={(jf) =>
