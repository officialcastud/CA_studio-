...
export default function PurchaseRegisterPage() {
  ...
  const registerRows = buildInventoryRegisterRows(entries, 'PUR');
  ...
  return (
    <div>
      <PageHeader title="Purchase Register" description="All purchase transactions with GST details">
        <div className="flex flex-col gap-2 items-end">
          <DateRangeFilter fromDate={fromDate} toDate={toDate} onDateChange={(f, t) => { setFromDate(f); setToDate(t); }} />
          <ExportButtons title="Purchase Register" companyName={company.name} entityType={entityLabel} dateRange={`${fromDate} to ${toDate}`} columns={exportColumns} data={exportData} pdfOrientation="landscape" />
        </div>
      </PageHeader>
      {loading ? (
        ...
        <InventoryRegisterFormat
          ...
          data={registerRows}
          totals={totals}
          emptyMessage="No purchase entries found for this period."
          getJfHref={(jf) =>
