...
export default function SalesRegisterPage() {
  ...
  const registerRows = buildInventoryRegisterRows(entries, 'SLS');
  ...
  return (
    <div>
      <PageHeader title="Sales Register" description="All sales transactions with GST details">
        <div className="flex flex-col gap-2 items-end">
          <DateRangeFilter fromDate={fromDate} toDate={toDate} onDateChange={(f, t) => { setFromDate(f); setToDate(t); }} />
          <ExportButtons title="Sales Register" companyName={company.name} entityType={entityLabel} dateRange={`${fromDate} to ${toDate}`} columns={exportColumns} data={exportData} pdfOrientation="landscape" />
        </div>
      </PageHeader>
      {loading ? (
        ...
        <InventoryRegisterFormat
          ...
          data={registerRows}
          totals={totals}
          emptyMessage="No sales entries found for this period."
          getJfHref={(jf) =>
