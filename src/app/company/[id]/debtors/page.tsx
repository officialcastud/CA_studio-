...
type ViewTab = 'summary' | 'ageing' | 'ledger';
...
export default function DebtorsPage() {
  ...
  const debtorBalances = useMemo(() => balances.filter(b => b.account_group === 'Trade Receivables' || b.account_group === 'Sundry Debtors'), [balances]);
  const ageingData = useMemo(() => computeDebtorAgeing(entries, toDate), [entries, toDate]);

  // Auto-open specific debtor ledger when ?account=Name is present
  useEffect(() => {
    const accountFromUrl = searchParams.get('account');
    if (accountFromUrl) {
      setSelectedDebtor(accountFromUrl);
    }
  }, [searchParams]);
  ...
  // If a debtor is selected, show their ledger
  if (selectedDebtor) {
    const ledgerRows = computeLedger(entries, selectedDebtor);
    ...
    return (
      <div>
        <PageHeader title={`Debtor: ${selectedDebtor}`} description="Debtor ledger detail">
          <div className="flex flex-col gap-2 items-end">
            <button
              onClick={() => {
                setSelectedDebtor(null);
                // Remove ?account from URL but keep other params intact
            ...
            <ExportButtons title={`Debtor Ledger - ${selectedDebtor}`} companyName={company.name} entityType={entityLabel} dateRange={`${fromDate} to ${toDate}`} columns={runningColumns} data={runningData} />
          </div>
        </PageHeader>
        {loading ? (
          ...
          <RegisterFormat title={`${selectedDebtor} — Ledger`} subtitle={`${fromDate} to ${toDate}`} companyName={company.name} columns={runningColumns} data={runningData} totals={{ debit: totalDebit, credit: totalCredit }} emptyMessage="No transactions found." />
        )}
      </div>
    );
  }

  // Summary columns for export
  const summaryColumns = [
    { header: 'S.No', key: 'sno' },
    { header: 'Debtor Name', key: 'account_name' },
  ...
  const summaryData = debtorBalances.map((b, i) => ({
    sno: i + 1,
  ...
  const totalOutstanding = debtorBalances.reduce((s, b) => s + (b.balance_type === 'Dr' ? b.balance : -b.balance), 0);
  ...
  return (
    <div>
      <PageHeader title="Debtors Ledger" description="Sundry Debtors with ageing analysis">
        <div className="flex flex-col gap-2 items-end">
          <DateRangeFilter fromDate={fromDate} toDate={toDate} onDateChange={(f, t) => { setFromDate(f); setToDate(t); }} />
          <div className="flex items-center gap-2">
            ...
            <ExportButtons title="Debtors Ledger" companyName={company.name} entityType={entityLabel} dateRange={`${fromDate} to ${toDate}`} columns={summaryColumns} data={summaryData} />
          </div>
        </div>
      </PageHeader>
      ...
      {loading ? (
        ...
        <AgeingFormat
          title="Debtors Ageing Analysis"
          companyName={company.name}
          asAtDate={toDate}
          data={ageingData}
          emptyMessage="No outstanding debtor balances found."
        />
      ) : (
        <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
          <div className="text-center py-3 border-b border-gray-200 bg-gray-50/50">
            ...
            <h3 className="text-base font-bold text-gray-900 mt-0.5">Sundry Debtors</h3>
            <p className="text-xs text-gray-400 mt-0.5">{fromDate} to {toDate}</p>
          </div>
          {debtorBalances.length === 0 ? (
            <div className="text-center py-14"><p className="text-sm text-gray-400">No debtor accounts found.</p></div>
          ) : (
              ...
              <table className="w-full text-sm">
                <thead className="sticky top-0">
                  <tr className="bg-gray-50 border-b border-gray-200">
                    ...
                    <th className="px-3 py-2.5 text-left text-[11px] font-semibold text-gray-500 uppercase tracking-wider">Debtor Name</th>
                ...
                <tbody>
                  {debtorBalances.map((b, i) => (
                    <tr
                      key={b.account_name}
                      className="border-b border-gray-100 hover:bg-blue-50 cursor-pointer"
                      onClick={() => setSelectedDebtor(b.account_name)}
                    >
                ...
                <tfoot>
                  <tr className="bg-gray-50 border-t-2 border-gray-300 font-bold">
                    ...
                    <td className="px-3 py-2 text-right font-mono">{formatIndianCurrency(Math.abs(totalOutstanding))} {totalOutstanding >= 0 ? 'Dr' : 'Cr'}</td>
                  </tr>
                </tfoot>
              </table>
            </div>
