      ...
      <div className="flex h-full w-full max-w-[1600px] max-h-[95vh] flex-col overflow-hidden rounded-2xl bg-white shadow-2xl">
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 bg-gray-50/30">
          <div className="mx-auto max-w-4xl space-y-5">
            {/* Header */}
            {state.kind === 'sales' ? (
              <HeaderSection kind="sales" invoice={state.invoice} updateInvoice={state.updateInvoice} mode={mode} />
            ) : (
            ...
            {config.showOriginalInvoice && (
              state.kind === 'sales' ? (
                <OriginalInvoiceSection kind="sales" invoice={state.invoice} updateInvoice={state.updateInvoice} existingInvoices={state.existingInvoices} selectOriginalInvoice={state.selectOriginalInvoice} mode={mode} />
              ) : (
            ...
            {config.multiLineItems && state.kind === 'sales' ? (
              ...
              <PurchaseItemSection fields={state.fields} updateField={state.updateField} mode={mode} />
            ) : null}
            ...
            {/* Error */}
            {state.error && (
              <div className="rounded-md border border-red-200 bg-red-50 px-4 py-3 text-sm font-semibold text-red-700 shadow-sm">{state.error}</div>
            )}
          </div>
        </div>
      </div>
